...
export const products = pgTable("products", {
  ...
  ...timestamps,
}, (t) => [uniqueIndex("products_merchant_slug_unique").on(t.merchantId, t.slug), uniqueIndex("products_merchant_external_unique").on(t.merchantId,t.sourceExternalId), index("products_catalog_idx").on(t.status, t.destination)]);
...
export const inventoryReservations = pgTable("inventory_reservations", { id: uuid("id").primaryKey().defaultRandom(), orderId: uuid("order_id").notNull().references(() => orders.id, { onDelete: "cascade" }), skuId: uuid("sku_id").notNull().references(() => skus.id), warehouseId: uuid("warehouse_id").notNull().references(() => warehouses.id), quantity: integer("quantity").notNull(), status: varchar("status", { length: 24 }).notNull().default("LOCKED"), expiresAt: timestamp("expires_at", { withTimezone: true }).notNull(), ...timestamps }, (t) => [index("inventory_reservation_order_idx").on(t.orderId), index("inventory_reservation_expiry_idx").on(t.status, t.expiresAt)]);
...
export const importJobRows = pgTable("import_job_rows",{id:uuid("id").primaryKey().defaultRandom(),jobId:uuid("job_id").notNull().references(()=>importJobs.id,{onDelete:"cascade"}),rowNumber:integer("row_number").notNull(),payload:jsonb("payload").notNull(),status:varchar("status",{length:24}).notNull(),errors:jsonb("errors").notNull().default([]),...timestamps},(t)=>[uniqueIndex("import_job_row_unique").on(t.jobId,t.rowNumber),index("import_job_rows_status_idx").on(t.jobId,t.status)]);
...
export const businessInquiries=pgTable("business_inquiries",{id:uuid("id").primaryKey().defaultRandom(),reference:varchar("reference",{length:32}).notNull(),company:varchar("company",{length:160}).notNull(),emailEncrypted:text("email_encrypted").notNull(),quantity:integer("quantity").notNull(),requirementsEncrypted:text("requirements_encrypted").notNull(),status:varchar("status",{length:24}).notNull().default("SUBMITTED"),...timestamps},(t)=>[uniqueIndex("business_inquiry_reference_unique").on(t.reference),index("business_inquiry_status_idx").on(t.status,t.createdAt)]);
